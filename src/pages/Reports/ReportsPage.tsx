...
const ReportsPage = () => {
  ...
  // Aumentado para evitar perdas por paginação do servidor em relatórios
  const pageSize = 1000;
  ...
  const { data: allRequests = [], isLoading } = useQuery({
    ...
    queryFn: () => {
      if (!import.meta.env.PROD) console.time('Carregar solicitações relatório');
      ...
      return getRequests(undefined, page, pageSize, statusFilter)
        .then((res) => {
          if (!import.meta.env.PROD) console.timeEnd('Carregar solicitações relatório');
          setError(null);
          return res?.data || [];
        })
        .catch((error) => {
          setError('Erro ao carregar solicitações do relatório. Veja o console para detalhes.');
          if (!import.meta.env.PROD) console.error('Erro ao buscar solicitações do relatório:', error);
          return [];
        });
    },
  });
  ...
  if (!import.meta.env.PROD) console.log('Renderizou ReportsPage');
                    ...
                    {filteredRequests.map((request: ITRequest) => {
                      ...
                      return (
                        <TableRow key={request.id}>
                          ...
                          <TableCell>{translate('type', request.type)}</TableCell>
                          <TableCell>{translate('priority', request.priority)}</TableCell>
                          <TableCell>{translate('status', request.status)}</TableCell>
