  ...
  try {
    ...
    for (const user of allUsers) {
      ...
      try {
        ...
        if (!import.meta.env.PROD) console.log(`Solicitação de manutenção preventiva criada para ${user.name}: ${newRequest.id}`);
      } catch (error) {
        if (!import.meta.env.PROD) console.error(`Erro ao criar solicitação para ${user.name}:`, error);
      }
    }
    
    if (!import.meta.env.PROD) console.log('Criação de solicitações de manutenção preventiva concluída.');
  } catch (error) {
    if (!import.meta.env.PROD) console.error('Erro geral na criação de solicitações de manutenção preventiva:', error);
  }
};
...
export const checkAndCreatePreventiveMaintenanceRequests = async (): Promise<void> => {
  if (isPreventiveMaintenanceDate()) {
    if (!import.meta.env.PROD) console.log('Data de manutenção preventiva detectada. Criando solicitações...');
